...
void _initializeBackgroundServices() async {
  ...
  try {
    ...
    // 🗺️ เริ่ม Location Topic Manager (ประหยัดค่าใช้จ่าย 99.995%!)
    _startLocationTopicManager();
...
/// 🗺️ **Location Topic Manager Starter**
/// เริ่ม Auto Subscribe Topics ตามตำแหน่ง เพื่อประหยัดค่าใช้จ่าย 99.995%
void _startLocationTopicManager() {
  // เริ่มหลังจาก app โหลดเสร็จ 3 วินาที เพื่อไม่รบกวน startup
  Future.delayed(const Duration(seconds: 3), () async {
    try {
      bool success = await LocationTopicManager.updateLocationTopics();
      if (success) {
        print('🎯 Location topics updated successfully - Cost optimized!');

        // แสดงข้อมูล topics ปัจจุบัน
        Map<String, dynamic> info =
            await LocationTopicManager.getCurrentTopicsInfo();
        print('📊 Current topics: ${info['topics']}');
        print('💰 Saving 99.995% compared to mass broadcasting!');
      } else {
        print('⚠️ Location topics update failed, will retry later');

        // Retry หลังจาก 30 วินาที
        Future.delayed(const Duration(seconds: 30), () {
          LocationTopicManager.updateLocationTopics();
        });
      }
    } catch (e) {
      print('❌ Error starting location topic manager: $e');
    }
  });
}
